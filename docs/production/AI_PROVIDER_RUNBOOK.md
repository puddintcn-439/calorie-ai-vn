...
- Kiểm tra GCP Console (IAM & Quotas) cho project chứa `GEMINI_API_KEY`: usage/quota, recent errors.
...
4) Kiểm tra chi tiết
- Xác nhận ứng dụng đang dùng đúng `GEMINI_API_KEY` và không bị expired/disabled.
