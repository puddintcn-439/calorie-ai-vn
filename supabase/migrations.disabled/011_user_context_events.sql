...
CREATE INDEX IF NOT EXISTS idx_user_context_events_user_id ON user_context_events(user_id);
CREATE INDEX IF NOT EXISTS idx_user_context_events_created_at ON user_context_events(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_user_context_events_context_mode ON user_context_events(context_mode);

-- Row Level Security
ALTER TABLE user_context_events ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view their own context events" ON user_context_events;
CREATE POLICY "Users can view their own context events" ON user_context_events
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own context events" ON user_context_events;
