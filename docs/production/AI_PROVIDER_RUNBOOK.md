...
- Nếu chỉ ảnh/receipt nhiều thành phần bị timeout, tăng `AI_IMAGE_PROVIDER_TIMEOUT_MS` trước; giữ `AI_PROVIDER_TIMEOUT_MS` cho text/coach để tránh treo request thường.
- Nếu latency ảnh cao nhưng provider không lỗi, dùng `AI_IMAGE_MODEL=gemini-2.5-flash-lite` cho fast-pass ảnh và giữ `AI_MODEL=gemini-2.5-flash` cho text/coach/refine.
