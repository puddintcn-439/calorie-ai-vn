    ...
    <TouchableOpacity style={styles.userCardPressable} onPress={() => router.push(`/admin/users/${encodeURIComponent(user.id)}` as any)}>
      <SurfaceCard style={styles.userCard}>
        <View style={styles.userHeader}>
          ...
          <View style={styles.planPill}><Text style={styles.planText}>{user.plan_tier}</Text></View>
        </View>
        <View style={styles.userStatsGrid}>
          <View style={styles.userStat}><Text style={styles.statLabel}>Status</Text><Text style={styles.statValue}>{user.subscription_status}</Text></View>
          <View style={styles.userStat}><Text style={styles.statLabel}>AI requests</Text><Text style={styles.statValue}>{formatNumber(user.total_ai_requests_month)}</Text></View>
          <View style={styles.userStat}><Text style={styles.statLabel}>Credits</Text><Text style={styles.statValue}>{formatNumber(user.credits_used_month)}</Text></View>
          <View style={styles.userStat}><Text style={styles.statLabel}>Food logs</Text><Text style={styles.statValue}>{formatNumber(user.food_logs_count)}</Text></View>
        </View>
...
export default function AdminUsersScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setResponse(null); setError(getAdminError(err));
    } finally {
      setLoading(false);
    }
  }, [search, plan, page]);
  ...
  const setPlanFilter = (nextPlan: string) => { setPage(1); setPlan(nextPlan); };
    ...
    <ScreenShell scroll scrollContentStyle={styles.scrollContent} reserveBottomNav={false}>
      ...
      <SurfaceCard style={styles.filterCard}>
        ...
        <View style={styles.searchRow}>
          <TextInput value={searchDraft} onChangeText={setSearchDraft} placeholder="Search by email" placeholderTextColor={theme.colors.textMuted} autoCapitalize="none" style={styles.searchInput} onSubmitEditing={applySearch} />
          <TouchableOpacity style={styles.filterButtonPrimary} onPress={applySearch}><Text style={styles.filterButtonPrimaryText}>Search</Text></TouchableOpacity>
        </View>
        <View style={styles.planRow}>
          {['', 'free', 'premium', 'pro'].map((item) => (
            <TouchableOpacity key={item || 'all'} style={[styles.planFilter, plan === item && styles.planFilterActive]} onPress={() => setPlanFilter(item)}><Text style={styles.planFilterText}>{item || 'all'}</Text></TouchableOpacity>
          ))}
        </View>
      </SurfaceCard>
      {loading ? (
        <SurfaceCard style={styles.centerCard}><ActivityIndicator color={theme.colors.accentMint} /><Text style={styles.mutedText}>Loading users...</Text></SurfaceCard>
      ) : error ? (
        <SurfaceCard style={styles.centerCard}><Text style={styles.errorTitle}>{error}</Text><Text style={styles.mutedText}>Your account must be included in ADMIN_EMAILS or BETA_ANALYTICS_ADMIN_EMAILS.</Text></SurfaceCard>
      ) : response ? (
        <View style={styles.content}>
          <View style={styles.resultHeader}><Text style={styles.sectionTitle}>Results</Text><Text style={styles.mutedText}>Page {response.page} · {formatNumber(response.total)} total</Text></View>
          {response.users.length === 0 ? <SurfaceCard style={styles.centerCard}><Text style={styles.mutedText}>No users found.</Text></SurfaceCard> : response.users.map((user) => <UserCard key={user.id} user={user} />)}
          <View style={styles.paginationRow}>
            <TouchableOpacity disabled={page <= 1} style={[styles.pageButton, page <= 1 && styles.pageButtonDisabled]} onPress={() => setPage((current) => Math.max(1, current - 1))}><Text style={styles.pageButtonText}>Previous</Text></TouchableOpacity>
            <TouchableOpacity disabled={!hasNextPage} style={[styles.pageButton, !hasNextPage && styles.pageButtonDisabled]} onPress={() => setPage((current) => current + 1)}><Text style={styles.pageButtonText}>Next</Text></TouchableOpacity>
          </View>
        </View>
      ) : null}
    </ScreenShell>
...
const styles = StyleSheet.create({
  ...
  userCardPressable: { borderRadius: 18 },
  userCard: { gap: 12 },
  ...
  planPill: { alignSelf: 'flex-start', borderRadius: 999, backgroundColor: theme.colors.accentMint, paddingHorizontal: 12, paddingVertical: 6 },
  planText: { color: theme.colors.textOnAccent, fontSize: 11, fontWeight: '900', textTransform: 'uppercase' },
  userStatsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  userStat: { flexGrow: 1, minWidth: 130, borderRadius: 14, backgroundColor: theme.colors.surfaceAlt, padding: 12 },
  ...
  pageButton: { borderRadius: 14, borderWidth: 1, borderColor: theme.colors.borderSubtle, paddingHorizontal: 16, paddingVertical: 10 },
