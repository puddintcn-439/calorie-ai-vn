...
-- Nutrition recommendation evidence metadata and versioned snapshots
...
alter table if exists public.users
  ...
  check (nutrition_target_snapshot is null or jsonb_typeof(nutrition_target_snapshot) = 'object');
...
begin
  ...
  then
    insert into public.daily_nutrition_target_history (
      user_id, target_date, algorithm_version, target_status, target_snapshot
    ) values (
      new.id,
      coalesce(nullif(new.nutrition_target_snapshot ->> 'date', '')::date, current_date),
